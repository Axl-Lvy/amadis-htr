...
accepted made the text better needs the gold set. Its sample is also
\corrPages{} of the \corrReachPagesWithSpan{} pages carrying a suspect span: a
...
\textbf{Three of five experiments are unmeasured.} E1, E3 and E4 need an
annotated gold set: twenty pages, roughly ten hours of one person's work, not
done. The consequence is specific and large --- \textbf{claim~C1 is
unevaluated}. This report shows that the pipeline runs, how fast, what its
accept rule refuses and how well it locates a passage. It does not show that
the fine-tune reads better than the models it replaced. Nor is the confusion
table the project has asserted for a year --- long~\qtr{ſ} against \qtr{f},
\qtr{u} against \qtr{n}, \qtr{c} against \qtr{e} --- quantifiable without a
reference transcription.

\textbf{The gold set will be small and single-annotator.} A difference smaller
than one person's disagreement with themselves is not a finding, and a second
annotator is the first thing a successor should add. Below 30 instances in a
cell, E3 and E4 degrade to a descriptive breakdown, and some cells will fall
below it.
